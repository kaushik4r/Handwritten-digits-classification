...
def nnObjFunction(params, *args):
    ...
    for i in range(trainingDataSize):
        trainingInput = training_data[i]
        
        #reference: https://github.com/ubdsgroup/ubmlcourse/blob/master/notebooks/PerceptronDemo.ipynb
        inputIncludingBiasTerm=np.hstack((trainingInput,np.ones(1.0)))  #size (d+1)*1
        ...
        outputclass = np.zeros(n_class) #initialize all output class to 0
        Label=int(training_label[i])
        outputclass[Label] = 1.0 # set class of true label
        ...
        grad_w2 = grad_w2 + (deltaOutput.reshape((n_class,1)) * np.hstack((hiddenLayerOutput,np.ones(1.0))))
        outputDeltaSum = np.dot(deltaOutput,w2)
        skipBiasHiddenNode = outputDeltaSum.shape[0] - 1
        outputDeltaSum = outputDeltaSum[0:skipBiasHiddenNode]
        delta_hidden = (1.0-hiddenLayerOutput) * hiddenLayerOutput * outputDeltaSum
        grad_w1 = grad_w1 + (delta_hidden.reshape((n_hidden,1)) * np.hstack((trainingInput,np.ones(1.0))))
    ...
    obj_val = Objective + ((lambdaval * randomization) / (2.0*trainingDataSize))
...
# Do not change this
def preprocess():
    pickle_obj = pickle.load(file=open('D:\\ML\\face_all.pickle', 'rb'))
...
pickle.dump(obj, open('D:\\ML\\params.pickle', 'wb'))
